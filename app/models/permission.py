...
class PermissionModel(db.Model):
    ...
    def get_permission(self, **kwargs):
        ...
        if search:
            result = allPermission.filter(or_(PermissionModel.name.like('%'+search+'%'),PermissionModel.code.like('%'+search+'%'))).order_by(desc(getattr(PermissionModel, sortBy))).offset(offset).limit(limit).all()
        elif filter:
            result = allPermission.filter_by(**filter).offset(offset).limit(limit).all()
        # this will sort by field name either asc or desc 
        elif sortBy:
            if orderBy == 'desc':
                result = allPermission.order_by(desc(getattr(PermissionModel, sortBy))).offset(offset).limit(limit).all()
            else:
